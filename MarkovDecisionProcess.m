%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%    based on https://www.mathworks.com/matlabcentral/fileexchange/55943-markov-decision-process-mdp-algorithm
...
Ra = -0.4; %reward in non-terminal states (used to initialise r[][])
...
R(xWin,yWin)=1;%Reward = 100
Pi(xWin,yWin)='+';%Policy
plot(xWin-.5,yWin-.5,'gd');
text(xWin-.9,yWin-.3,'Winning +1')

%Loss point
xLos=4;%X Coordinate of the Loss point
yLos=2;%Y Coordinate of the Loss point
R(xLos,yLos)=-1;%Reward = -100
Pi(xLos,yLos)='-';%Policy
plot(xLos-.5,yLos-.5,'rd');
text(xLos-.8,yLos-.4,'Loss -1')


R(2,2)=-0.5;%Reward = -0.5
R(4,1)=0.2;%Reward = 0.2
...
deltaMin = 1e-6; %convergence criterion for iteration
delta = 0;
